...
{
	int i,prev,fstp,totframe,nlines;
	char null[1024];
	...
	*intrj>>fstp; i=fstp;
	...
	nlines=std::count(std::istreambuf_iterator<char>(*intrj), std::istreambuf_iterator<char>(), '\n');
	totframe=(int)nlines/(nmovatm+2);
	...
	while(!intrj->eof()) {
		totframe++;
		if(!iflag) byte_offset[totframe].xyz = intrj->tellg();
		else byte_offset[totframe].coord = intrj->tellg();
		for(i=0;i<nmovatm+2;i++) intrj->getline(null,1024);
	}
	//totframe--;
	lmptmp = totframe;
