...
void GROUP::clone(GROUP * grp2)
{

	int i;
	natom       = grp2->natom;
	nmol        = grp2->nmol;
	constraint  = grp2->constraint;
	rotsym      = grp2->rotsym;
	linear      = grp2->linear;
	chg         = grp2->chg;
	mass        = grp2->mass;
	vol         = grp2->vol;
	eng.avg     = grp2->eng.avg;
	eng.sum     = grp2->eng.sum;
	eng.nXY     = grp2->eng.nXY;
	eng.npt     = grp2->eng.npt;
	eng.std     = grp2->eng.std;
	eng.sigma2x = grp2->eng.sigma2x;
	init_atom();
	for(i=0;i<natom;i++)
		atm[i] = grp2->atm[i];
