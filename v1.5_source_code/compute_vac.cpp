...
//enum{onePT,twoPT,twoPTMOL,twoPTMOLMF,twoPTMOLINT}
...
ComputeVAC::~ComputeVAC()
{
	...
	memory->destroy_2d_double_array(fmf);
	...
	fftw_free(out);
	if(trj_atom_eng) {
		for(i=0;i<natom;i++) delete [] atomEng[i];
		delete [] atomEng;
	}
...
void ComputeVAC::init(MODEL *model)
{
	...
	has_grp_file_eng = model->ctl.has_grp_file_eng;

	per_mol_thermo = model->ctl.ana_vac_per_mol_flag;

	check_grp_eng = model->ctl.ana_vac_check_grp_eng;

	vac_vsfactor = model->ctl.ana_vac_vsfactor;
    ...
    switch(c2pt) {
		...
		case 1: cout<<" Mode analysis with 2PT corrections will be performed"<<endl; c2pt=1; cmol=0; c2ptmf=0; break;
		case 2: cout<<" Mode analysis with 2PT-MF corrections will be performed"<<endl; c2pt=1; cmol=0; c2ptmf=1; break;
		case 3: cout<<" Mode analysis with molecular corrections will be performed"<<endl; c2pt=0; cmol=1; break;
		case 4: cout<<" Mode analysis with molecular and 2PT corrections will be performed"<<endl; c2pt=cmol=1; break;
		case 5: cout<<" Mode analysis with molecular and 2PT-MF corrections will be performed"<<endl; c2pt=cmol=c2ptmf=1; break;
		case 6: cout<<" Mode analysis with molecular internal coordinates and 2PT corrections will be performed"<<endl; c2pt=cmol=1; break;
	}

	if(!cmol) vactype=1; 
	else vactype=VELTYPE+1; 
	model->find_molingrp();
	if(cmol&&per_mol_thermo) {
		make_mols_grps(model);
		model->find_molingrp();
		check_grp_eng = 0;
		cout<<" Will report Thermodynamics of each molecule in each group seperately"<<endl;
	}
	fix_missing_grp_mol_entry(model);
	//if(nthreads > 1) 
	//	cout<<" Will use "<<nthreads<<" OMP threads to parallelize FFTs"<<endl;
...
void ComputeVAC::mem_usage(MODEL *model)
{
	...
	//calc the memory needed to read in coordinates of an atom from the whole trj
	double memperatm=0.0;
	for(i=0;i<=vactype;i++) {
		memperatm += sizeof(float***);
		for(j=0;j<vacnsteps;j++) {
			memperatm += sizeof(float **);
			for(k=0;k<vacatmperpass;k++) {
				memperatm += sizeof(float *);
				for(l=0;l<3;l++) memperatm += sizeof(float);
			}
		}
	}
	memperatm *=(vactype/MEGABYTE);
...
void ComputeVAC::allocate()
{
	...
	for(i=0;i<vactype;i++) {
		vacvv[i] = new float **[vacnsteps];
		for(j=0;j<vacnsteps;j++) {
			vacvv[i][j] = new float *[vacatmperpass];
			for(k=0;k<vacatmperpass;k++) {
				vacvv[i][j][k] = new float [3];
			}
		}
	}
	...
	fmf = memory->create_2d_double_array(2,k,"ComputeVAC::vacfmf");
	...
	thermo = memory->create_3d_double_array(15,ngrp,vactype,"ComputeVAC::thermo");
	...
	for(i=0;i<15;i++)
	 ...
	 if(trj_atom_eng) {
		atomEng = new float *[natom];
		for(i=0;i<natom;i++) {
			atomEng[i] = new float [nframe];
			for(j=0;j<nframe;j++)
				atomEng[i][j]=0.0;
		}
	}
...
{ 
	...
	if(cmol==0) {
		for(i=0;i<vacatmpass;i++) {
			id=vacread[i];
			for(k=0;k<3;k++) { 
				vacvv[0][iframe][i][k]=(float)(model->atom[id].vel[k]*vac_vsfactor);
			}
		}
	} else {
		model->cal_vcomp();
		for(i=0;i<vacatmpass;i++) {
			id=vacread[i];
			for(k=0;k<3;k++) {
				vacvv[vtrans][iframe][i][k]=(float)(model->atom[id].vt[k]*vac_vsfactor);
				vacvv[vrotat][iframe][i][k]=(float)(model->atom[id].vr[k]*vac_vsfactor);
				vacvv[vimvib][iframe][i][k]=(float)(model->atom[id].vv[k]*vac_vsfactor);
				vacvv[vtotal][iframe][i][k]=(float)(model->atom[id].vel[k]*vac_vsfactor);
			}
		} 
		id=vaclastmol-vacmolpass; //first molecular id
		for(i=id;i<vaclastmol;i++) //i is the molecular id
			for(k=0;k<3;k++) 
				vacvv[vangul][iframe][i-id][k]=(float)(model->mol[i].anguv[k]*vac_vsfactor);
		if (vacipass==0)
			for(i=0;i<model->nmol;i++) 
	...
	if (vacipass==0) {
		...
		if(! model->ctl.ana_vac_eng_flag && !trj_atom_eng) model->grp[ngrp-1].eng.add_data(model->prp.Et);
		vacP+=model->prp.P;
	...
	if (trj_atom_eng) {
		...
		for(i=0;i<vacatmpass;i++) {
			id=vacread[i];
			atomEng[id][iframe] = model->atom[id].eng_tmp;
			tot+=model->atom[id].eng_tmp;
		}
...
{ 
	...
	if (trj_atom_eng) {
		for (i=0;i<ngrp-1;i++) {
			for(j=0;j<nframe;j++) {
				...
				model->grp[i].eng.add_data(grpEng);
			}
		}
	}
	...
	//apply 1PT/2PT/2PT-MF correction
	do_2pt(outf,model);
...
void ComputeVAC::calc_thermo_vals(ostream *outf, MODEL *model) 
{
	...
	//energy
	k = vactype - 1;
	eng_tot = vacE[ngrp-1][k]/nframe;
	if(ngrp==1||(!has_atom_eng&&!has_grp_file_eng)) {
		...
		for(i=0;i<ngrp-1;i++) { //distribute total energy to each grp based on dof (ideal gas/perfect solid approximation)
			...
			vacE[i][k] = model->grp[i].eng.sum = vacE[ngrp-1][k] * vacDF[i][k]/vacDF[ngrp-1][k];
			model->grp[i].eng.sigma2x = model->grp[ngrp-1].eng.sigma2x * vacDF[i][k]/vacDF[ngrp-1][k];
		}
	} else {
		for(i=0;i<ngrp;i++) {
			if (! model->ctl.ana_vac_eng_flag&&!has_grp_file_eng) model->grp[i].eng.cal_avg();
			vacE[i][k] = model->grp[i].eng.avg;
		}
	}
	...
	scale_f = 1.0;
	if(fabs(ediff)>0.001&&ngrp>1&&check_grp_eng) {
...
{ 
	...
	float pspec_ave,pspec_adev,pspec_sdev,pspec_var,pspec_skew,pspec_curt; //moments of power spectrum
	...
	if (!c2pt) cout<<"Applying 1PT correction...";
	else if(c2pt&&!cmol) cout<<"Applying 2PT correction...";
	else if(c2pt&&cmol&&!c2ptmf) cout<<"Applying 2PT molecular correction...";
	else if(c2pt&&cmol&&c2ptmf) cout<<"Applying 2PT molecular memory function correction...";
	fflush(stdout);
	...
	for(j=0;j<ngrp;j++) {
		...
		if(c2ptmf) {
			//first get moments on translations
			moment(pwr[0][j],nused,&pspec_ave,&pspec_adev,&pspec_sdev,&pspec_var,&pspec_skew,&pspec_curt);
			if (DEBUG) {
				cout<<"trans power spectrum"<<endl;
				cout<<"average (1st moment): "<<pspec_ave<<endl;
				cout<<"average deviation: "<<pspec_adev<<endl;
				cout<<"standard deviation: "<<pspec_adev<<endl;
				cout<<"variance (2nd moment): "<<pspec_var<<endl;
				cout<<"sknewness (3rd moment): "<<pspec_skew<<endl;
				cout<<"kurtosis (4th moment): "<<pspec_curt<<endl;
			}
			exit(1);
		...
		fmf[0][j]=f2pt[0][j]; //2pt-MF
		if(c2ptmf) twoPTmf(pwr[0][j],pwr[0][j][0],nused,pwrfreq,ngmol,f2pt[0][j],fmf[0][j]); //determine the value of fmf

		if (f2pt[0][j]==fmf[0][j]) ry[0]=pow(f2pt[0][j]/K2pt[0][j],1.5); //packing fraction y, eqn 32
		...
		if(rT[0]>0.0) {
			...
			hsdf[1][j]=HSDF(pwr[1][j],nused,pwrfreq,ngmol,f2pt[1][j],fmf[0][j]); /*determine hard sphere degrees of freedom hsdf*/
		...
		for(i=0;i<nused;i++) {
			twoPT(&tmpg,&tmps,pwr[0][j][0],pwr[0][j][i],pwrfreq*i,ngmol,f2pt[0][j],fmf[0][j]); //get frequency dependent hs DoS - eqn 24
			if(!c2pt) tmpg=0.0; //1PT -ignore gas phase contribution
			...
			if(rT[0]>0&&c2pt==1) twoPT(&tmpg,&tmps,pwr[1][j][0],pwr[1][j][i],pwrfreq*i,ngmol,f2pt[1][j],fmf[1][j]); 
			else tmpg=tmps=0;
			if (cmol) {
				if(!c2pt) tmpg = 0.0;
...
{ 
	...
	*outf<<endl; outthermo<<endl;
	...
	for(j=0;j<ngrp;j++)
		for(k=0;k<vactype;k++)
			thermo[i][j][k] = thermo[6][j][k]-thermo[2][j][k]+thermo[10][j][k];
	...
	//print_thermo((char *)"Aq_E(kJ/mol/SimBox)",14,show2pt,outf,&outthermo);
...
/* ---------------------------------------------------------------------- */
void ComputeVAC::make_mols_grps(MODEL *model)
{
	int c,i,j,k,l,m,n;
	...
	if (c == 0) c = 1;
	for (i=0;i<c;i++) {
		if(model->grp[i].nmol<2) continue;
		n+=model->grp[i].nmol;
	}
	if(n>ngrp) {
		newgrp = new GROUP [n];
		for (i=0;i<c;i++) newgrp[i].clone(&model->grp[i]);
		newgrp[n-1].clone(&model->grp[c]);
		k=ngrp-1;
		for (i=0;i<c;i++) {
			if(model->grp[i].nmol<2) continue;
			for(j=0;j<model->grp[i].nmol;j++) {
				newgrp[k].nmol       = 1;
				newgrp[k].constraint = model->grp[i].constraint/model->grp[i].nmol;
				newgrp[k].rotsym     = model->grp[i].rotsym;
				newgrp[k].linear     = model->grp[i].linear;
				newgrp[k].vol        = model->grp[i].vol/model->grp[i].nmol;

				l = model->grp[i].mol[j];
				newgrp[k].chg        = model->mol[l].chg;
				newgrp[k].mass       = model->mol[l].mass;

				//energy
				newgrp[k].eng.avg    = model->grp[i].eng.avg/model->grp[i].nmol;
				newgrp[k].eng.sum    = model->grp[i].eng.sum/model->grp[i].nmol;
				newgrp[k].eng.std    = sqrt(pow(model->grp[i].eng.std,2)/model->grp[i].nmol);
				newgrp[k].eng.sigma2x= newgrp[k].eng.std*newgrp[k].eng.std;
				newgrp[k].eng.nXY    = 1;
				newgrp[k].eng.npt    = 1;

				//atom data
				newgrp[k].natom      = model->mol[l].natom;
				newgrp[k].init_atom();
				for(m=0;m<model->mol[l].natom;m++) 
					newgrp[k].atm[m]  = model->mol[l].atm[m];
				k++;
			}
		}
		ngrp = n;
