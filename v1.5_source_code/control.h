...
public:
   ...
   int    ana_vac_per_mol_flag; //split the thermodynamics into per molecule vals for each grp
   ...
   int    ana_vac_mol_flag;    //molecular option flag for 2pt calcs
   ...
   int    has_grp_file_eng;
   int    ana_vac_check_grp_eng;

   int    ana_vac_2pt; //flag for 2pt calculations, 0:no, 1:yes, 2:2pt for molecules
   int    ana_vac_method; //type of 2pt calculation to perform: 0 - 1PT, 1 - 2PT (LBG), 2 - 2PT-MF (Sandia Group)
   ...
   double ana_vac_vsfactor; //velocity scaling factor for (xyz) trajectories
