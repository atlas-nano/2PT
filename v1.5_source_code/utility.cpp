...
/* ---------------------------------------------------------------------- */
void twoPT(double *tmpg,double *tmps,double s0,double sv,double v,int nmol,double fract_f,double fmf)
{
	if(fract_f==0) { *tmps=sv; *tmpg=0; }
	else if(v==0) { *tmpg=s0; *tmps=0; }
	else {
		if(fract_f==fmf) *tmpg=s0/(1.0+pow(PI*s0*v/(6.0*nmol*fract_f),2.0)); //eqn 24
...
/* The following functions implements the memory function approach of 2PT
 * MP DESJARLAIS PHYSICAL REVIEW E 88, 062145 (2013)
 * Adapted from code from ST Lin Aug 2016 */
...
{   
	...
	if(DEBUG) {
		printf("2PTMF: A %f B %f f2pt %f fmf %f s0 %f nmol %d\n",A,B,f2pt,fmf,s0,nmol);
	...
	while(dev>1e-8) {
		...
		//if(DEBUG) printf("loop=%d fmf=%e nxtfmf=%e dev=%e",cc,fmf,nxtfmf,dev); cout<<endl;
...
standard  deviation sdev, variance var,skewness skew, and kurtosis curt,i.e. the 1st,2nd,3rd and 4th moments
*/
void moment(double *data, int n, float *ave, float *adev, float *sdev, float *var, float *skew, float *curt)
{
	int j;
	float ep=0.0,s,p;
	if (n <= 1) { cout<<"n must be at least 2 in moment"<<endl; exit(1); }
	s=0.0;
	...
	*sdev=sqrt(*var);
	...
	} else { cout<<"No skew/kurtosis when variance = 0 (in moment)"<<endl; exit(1); }
}
