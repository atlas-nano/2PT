...
CONTROL::CONTROL () 
{
  ...
  ana_vac_2pt=1; //2pt calculations 0:2PT for monoatomic systems, 3:2PT for molecules
...
int CONTROL::rd_ctl(char *ctlf)
{
  ...
  while(!inctl.eof()) {
    if(opt[0]=='#'||opt[0]=='*'||opt[0]=='!'||opt[0]=='/') { inctl.getline(opt,1024); }
      ...
      else if (strcmp(opt,"IN_XYZTRJ")==0) {cout<<"GOT HERE"<<endl;   in_trj_flag=xyztrj; }
    ...
    else if (strcmp(opt,"ANALYSIS_MOLECULE_FLAG")==0) { inctl>>ana_vac_2pt; if(ana_vac_2pt>1||ana_vac_2pt<0) ana_vac_2pt=0; }
    else if (strcmp(opt,"ANALYSIS_VAC_2PT")==0) { inctl>>ana_vac_2pt; //backward compatibility
      if(ana_vac_2pt<2) ana_vac_2pt = 0;
      else ana_vac_2pt = 1;
    } 
...
void CONTROL::out_ctl(ostream *outf)
{
  ...
  *outf<<null<<endl;
  ...
  if(in_trj_flag==lmptrj) {
    ...
    *outf<<null<<endl;
  }else if (in_trj_flag != c2trj && ana_vac_2pt) {
  ...
  *outf<<null<<endl;
  sprintf(null,"%-40s %d","ANALYSIS_MOLECULE_OPTION",ana_vac_2pt);
  *outf<<null<<endl;
